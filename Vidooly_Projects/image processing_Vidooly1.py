...
# does the image processing and splits the data into train and test sets
...
def scale(annot, im):   
    ...
    for s in scales:
        w = int(rect['wid'] * s)
        h = int(rect['hgt'] * s)
        cropped_im = im.crop((rect['cx'] - w // 2, rect['cy'] - h // 2,rect['cx'] + w // 2, rect['cy'] + h // 2))
        resized_im = cropped_im.resize((width, height))
...
def save_im(annot, cnt, *args):         
    ...
    for i, arg in enumerate(args):
        for im, suffix in zip(arg[0], arg[1]):
            save_fn = '_'.join([
                fn.split('.')[0], class_name, train_subset_class, str(cnt),suffix]) + os.path.splitext(fn)[1]
...
def crop_and_aug(annot_train):     
    ...
    for annot in annot_train:
        ...
        # save images
        save_im(annot, cnt_per_file[fn], [cropped_ims, cropped_suffixes],[shifted_ims, shifted_suffixes],[scaled_ims, scaled_suffixes],[rotated_ims, rotated_suffixes])
...
def do_train_test_split():                                   
    ...
    for class_name in class_names:                           # create directories under a particular class name.
        if os.path.exists(os.path.join(pp_dir, class_name, 'train')):
            continue
        if os.path.exists(os.path.join(pp_dir, class_name, 'test')):
            continue

        imgs = [img for img in os.listdir(os.path.join(pp_dir, class_name))]
...
# prints annotation text file info
def main():
