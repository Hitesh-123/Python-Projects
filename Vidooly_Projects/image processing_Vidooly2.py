...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 23:39:52 2018
...
pp_dir = os.path.join(dir, 'processed')               # Directory where processed images are stored
...
def load_logo(data_dir):    
    ...
    for image in image_files:
        ...
        try:
            image_data = (ndimage.imread(image_file).astype(float) - pix_val / 2) / pix_val
...
def combine(pickle_files, train_size, val_size=0):     
    ...
    valid_dataset, valid_labels = array(val_size, width, height, channel)
    train_dataset, train_labels = array(train_size, width, height, channel)
...
def main():  
    dirs = [os.path.join(pp_dir, class_name, 'train')      # Look into all the train folder of the class
        for class_name in clas.CLASS_NAME]
    test_dirs = [os.path.join(pp_dir, class_name, 'test')        # Look into all the test folder of the class
        for class_name in clas.CLASS_NAME]
